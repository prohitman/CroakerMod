		...
		if (this.operation == MoveControl.Operation.STRAFE) {
			...
			if (this.side != null && this.block != null) {
				...
				if (targetDist < 0.0001D) {
					...
					if (jumpDir == null && this.side != null && Math.abs(this.climber.getGroundDirection().getRight().y) > 0.5f && (!this.climber.canAttachToSide(this.side) || !this.climber.canAttachToSide(Direction.getNearest(dx, dy, dz))) && this.wantedY > this.mob.getY() + 0.1f && verticalOffset > this.mob.getStepHeight()) {
